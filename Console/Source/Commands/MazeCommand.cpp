...
MazeCommand::MazeCommand(Console::Context& context) noexcept: Command {context} {
	...
	mazeGenerators.emplace_back(std::make_unique<Core::WilsonsMazeGenerator>());
}
...
void MazeCommand::execute(const std::unordered_map<std::string, std::string>& parameters) {
	...
	try {
		...
		auto generator {std::find_if(mazeGenerators.begin(),
		                             mazeGenerators.end(),
		                             [&algorithm](const auto& mazeGenerator) {
